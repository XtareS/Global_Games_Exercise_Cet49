...
namespace Global_Games_Exercise_Cet49.Controllers
{
    ...
    public class RegistosController : Controller
    {
        ...
        public async Task<IActionResult> Create([Bind("Id,ImageUrl,ImageFile,Name,Sur,Adress,Local,CC,Birth,Email")] Registo registo)
        {
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> Edit(int id, [Bind("Id,ImageUrl,ImageFile,Name,Sur,Adress,Local,CC,Birth,Mail")] Registo registo)
        {
            if (id != registo.Id)
            {
                return NotFound();
